...
export class EditableArticleArray {
    constructor(parentProps, array) {
        this.parentProps = parentProps
        this.array = (array[0].year && array[0].title && array[0].journal) ? array : [{ year: '', title: '', journal: '' }];
    ...
    setView() {
        appendChildren(detachChildren(this.view), [
            this.array.map((obj, index) => {
                ...
                appendChildren(container, [
                    addEvent(addClasses(createButton('Eliminar Articulo'), 'editableArray_removeButton'), () => { this.removeObjectAtIndex(index) }) // Add button
                ]);
                return container;
            }),
