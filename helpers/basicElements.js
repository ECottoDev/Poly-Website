...
export function removeNumbers(inputString) {
    return inputString.replace(/\d+/g, '').toUpperCase().replace(/\s+/g, '_');
}

/**
 * Extracts the number from a string inside parentheses.
 * @param {string} str - The input string containing the number inside parentheses.
 * @returns {number} - The extracted number or null if no match is found.
 */
...
/**
 * Replaces numbers inside parentheses with buttons that execute a callback when pressed.
 * @param {string} str - The input string containing numbers inside parentheses.
 * @param {function} callback - The callback function to execute when a button is pressed.
 * @returns {string} - The formatted string with buttons.
 */
/**
 * Replaces numbers inside parentheses with buttons that log the number when pressed.
 * @param {string} str - The input string containing numbers inside parentheses.
 * @returns {string} - The formatted string with buttons.
 */
export function replaceWithButtons(str, callback = () => { }) {
    const regex = /\((\d+)\)/g;
    let result = '';
    let lastIndex = 0;
    let match;
    ...
    while ((match = regex.exec(str)) !== null) {
        ...
        const buttonId = `button-${number}-${Math.random().toString(36).substr(2, 9)}`;
        result += `<button class="replacedButton" id="${buttonId}">(${number})</button>`;
        lastIndex = regex.lastIndex;

        // Add event listener after the button is added to the DOM
        setTimeout(() => {
            document.getElementById(buttonId).addEventListener('click', () => callback(number));
        }, 0);
    }
...
/**
 * Adds an ID to the given element.
 * @param {Element} element - The DOM element to which the ID will be added.
 * @param {string} id - The ID to be added to the element.
 * @returns {Element} - The DOM element with the added ID.
 */
...
/**
 * Jumps to the location of the given ID.
...
export function setUsernameCookie(username) {
    const d = new Date();
    d.setTime(d.getTime() + (1 * 60 * 60 * 1000)); // 1 hour in milliseconds
    const expires = "expires=" + d.toUTCString();
    document.cookie = "username=" + username + ";" + expires + ";path=/";
}
export function getUsernameCookie() {
    const name = "username=";
    const decodedCookie = decodeURIComponent(document.cookie); // Decodes the cookie string
    const cookieArray = decodedCookie.split(';'); // Split the cookies
    for (let i = 0; i < cookieArray.length; i++) {
        let cookie = cookieArray[i].trim();
        if (cookie.indexOf(name) === 0) {
            return cookie.substring(name.length, cookie.length); // Return the value of the username cookie
        }
    }
    return ""; // Return empty string if the username cookie is not found
}
export function deleteUsernameCookie() {
    document.cookie = "username=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
}
export function updateUsernameCookieExpiration(user) {
    const date = new Date();
